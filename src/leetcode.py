...
Ref: https://github.com/tusharsadhwani/python_leetcode_runner
"""
...
def run_leetcode_solution(filepath: str) -> int:
    ...
    try:
        ...
        pattern = r"src/\d+\."
        module_name = (
            re.sub(pattern, "", filename).replace(".py", "")
    ...
    """
    NOTE: Improved here, got the latest method
    ...
    method_name = method_names[mi]
    info_color = color.BOLD + color.INFO
    print_colored(f"==== {method_name} ====", clr=info_color)
...
def print_failed_testcases(
    ...
    for _traceback, inputs, expected, output in failed_testcases:
        inputs_string = ", ".join(map(str, inputs))
        assertion = get_assert_statement(_traceback)
